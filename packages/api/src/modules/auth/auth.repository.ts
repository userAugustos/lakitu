...
async function createUser(input: NewUserRow): Promise<UserRow> {
...
async function createChallenge(input: NewAuthChallengeRow): Promise<AuthChallengeRow> {
  const inserted = await db.insert(authChallenges).values(input).returning();
  return inserted[0]!;
}

async function findValidChallengeByCode(
  userId: string,
  codeHash: string
